...
public:
	...
	void set_transition(int vertex_from, int vertex_to, string trans_symbol) {
		trans new_trans;
	...
	void read_production(char lhs, string rhs_term, char rhs_nonterm) {
...
FSM read_grammar(int no_of_productions) {
	...
	for(int i = 0; i < no_of_productions; i++) {
		...
		if(rhs.find('.') == string::npos) {
			...
			rhs_term = rhs.substr(0, rhs.size() - 2);
